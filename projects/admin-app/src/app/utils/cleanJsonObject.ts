...
export function extractSecondSpanContent(htmlString: string, chapitreId: number): string {
  ...
  const spanMatches = htmlString.match(
    new RegExp(`<span id=['"]paragraphe-${chapitreId}['"]>(.*?)</span>`, 's'),
  );
...
export function extractByPositionH4Title(texte: string, x: number): string {
  console.log(
    `Extraction du titre H4 du paragraphe ${x} (longueur de l'article : ${texte?.length || 0} caractères)`,
  );
