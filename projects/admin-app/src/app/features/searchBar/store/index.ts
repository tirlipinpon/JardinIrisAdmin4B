...
export interface SearchState {
  ...
  articleValid: {
    ...
    explication: { raisonArticle1: string | null };
  };
...
export const SearchStore = signalStore(
  ...
  withComputed(store => ({
    ...
    isPostDescriptionMeteo: computed(() => {
      ...
      return (
        post !== null &&
        post.description_meteo !== undefined &&
        post.description_meteo !== null &&
        post.description_meteo !== ''
      );
    }),
    ...
    isPostPhraseAccroche: computed(() => {
      ...
      return (
        post !== null &&
        post.phrase_accroche !== undefined &&
        post.phrase_accroche !== null &&
        post.phrase_accroche !== ''
      );
    }),
    ...
    isPostLienUrlArticle: computed(() => {
      ...
      return (
        post !== null &&
        post.lien_url_article !== undefined &&
        post.lien_url_article !== null &&
        post.lien_url_article.lien1 !== ''
      );
    }),
  ...
  withMethods((store, infra = inject(SearchInfrastructure)) => ({
      ...
      pipe(
        ...
        switchMap(() => {
          ...
          return infra.selectArticle(getArticles).pipe(
            tapResponse({
              next: articleValid =>
                patchState(store, { articleValid: articleValid, isLoading: false }),
              error: error => patchState(store, { isLoading: false }),
            }),
          );
        }),
    ...
    generateArticle: rxMethod<string | undefined>(
      pipe(
        ...
        switchMap((input?: string) => {
          ...
          if (input?.length) {
            return processGeneration(input);
          }
          const articleValid = store.getArticleValid();
          if (articleValid.valid && articleValid.url) {
            return processGeneration(articleValid.url);
          }
          const ideaPost = store.getIdeaPost();
          if (ideaPost?.id !== null && ideaPost?.description) {
            return processGeneration(ideaPost.description);
          }
          patchState(store, { isLoading: false });
          return EMPTY;
        }),
      ...
      pipe(
        ...
        map((url: string) => {
          ...
          const updatedPost = currentPost
            ? { ...currentPost, lien_url_article: { lien1: url } }
            : { lien_url_article: { lien1: url } };
          patchState(store, {
            post: updatedPost,
            isLoading: false,
          });
      ...
      pipe(
        ...
        switchMap(() => {
          ...
          return infra.formatInStructure(generatedArticle, 'UPGRADE').pipe(
            tapResponse({
              next: upgradedArticle =>
                patchState(store, { articleUpgraded: upgradedArticle, isLoading: false }),
      ...
      pipe(
        ...
        switchMap(() => {
          return infra.getPostTitreAndId().pipe(
            tapResponse({
              next: postTitreAndId =>
                patchState(store, { postTitreAndId: postTitreAndId, isLoading: false }),
              error: () => patchState(store, { isLoading: false }),
            }),
          );
        }),
    ...
    addInternalLinkByChapter: rxMethod<void>(
      pipe(
        tap(() =>
          updateState(store, '[addInternalLinkByChapter] update loading', { isLoading: true }),
        ),
        switchMap(() => {
          ...
          if (!getPostTitreAndId) {
            patchState(store, { isLoading: false });
            return EMPTY;
          }
          return infra.formatInStructure(getArticleHtml, 'LINK', getPostTitreAndId).pipe(
            tapResponse({
              next: articleLinkAdded =>
                patchState(store, { articleLinkAdded: articleLinkAdded, isLoading: false }),
              error: () => patchState(store, { isLoading: false }),
            }),
          );
        }),
    ...
    updateIdeaPost: rxMethod<void>(
      pipe(
        ...
        switchMap(() => {
          ...
          return infra.updateIdeaPost(ideaPost.id as number, postId)
        }),
      ),
    ),
    addImagesInArticle: rxMethod<void>(
      pipe(
        ...
        switchMap(() => {
          ...
          return infra.addImagesInArticle(getPost, getPostId)
        }),
      ),
    ),
