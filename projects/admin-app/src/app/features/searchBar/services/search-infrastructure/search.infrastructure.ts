...
@Injectable({
  ...
  useFactory: () => {
    ...
    return new SearchInfrastructure(
      theNewsApiService,
      openaiApiService,
      getPromptsService,
      supabaseService,
...
export class SearchInfrastructure {
  textPromptImage =
    "créé moi une image avec peu d'éléments ', concentre toi sur le sujet que je vais te donner, car cette image vas allez comme illustration d'un blog, et ne met surtout aucun de texte sur l'image ni humain ou partie humaine , voici le sujet : ";
  ...
  selectArticle(articles: { url: string; image_url: string }[]): Observable<{
    valid: boolean | null;
  ...
  searchIdea(): Observable<{ id: number | null; description: string | null }> {
    return from(
      this.supabaseService.getFirstIdeaPostByMonth(
        new Date().getMonth() + 1,
  ...
  formatInStructure(
    article: string,
    type: string,
    postTitreAndId?: { titre: string; id: number }[],
  ): Observable<string> {
        ...
        return from(chapitreIds).pipe(
          mergeMap(chapitreId => {
            ...
            if (type === 'HTML') {
              ...
              prompt = this.getPromptsService.getPromptGenericAddInternalLinkInArticle(
                chapitreText,
                postTitreAndId,
              );
            }
            ...
            return from(this.openaiApiService.fetchData(prompt, true)).pipe(
              map(upgradedText => {
                ...
                if (upgradedText === null) {
                  console.warn(
                    `Le chapitre ${chapitreId} n'a pas pu être amélioré, texte reçu:`,
                    upgradedText,
                  );
                  return fullArticle; // Retourne l'article sans modifier ce chapitre
                }
                const upgradedTextJson: { upgraded: string; idToRemove?: number } = JSON.parse(
                  extractJSONBlock(upgradedText),
                );
  ...
  addImagesInArticle(getPost: string, getPostId: number): Observable<{ success: boolean }> {
    ...
    return from(
      this.addImagesToChaptersService.getKeyWordsFromChapitreInArticleAndSetImageUrl(
        getPost,
        getPostId,
      ),
    );
  ...
  async generateImageIa(description: string, postId: number) {
    let image_url = await this.openaiApiService.imageGenerartor(
      this.getPromptsService.getOpenAiPromptImageGenerator(description),
    );
