...
import {
  extractByPositionH4Title,
  extractJSONBlock,
  extractSecondSpanContent,
} from '../../../../utils/cleanJsonObject';
...
export class AddImagesToChaptersService {
  ...
  async getKeyWordsFromChapitreInArticleAndSetImageUrl(article: string, articleId: number) {
    const chapitreKeyWordList: string[] = [];
    for (let i = 1; i <= 6; i++) {
      ...
      console.log(
        `Chapitre ${chapitreId} - Paragraphe extrait:`,
        extractedParagraphe?.substring(0, 50) + '...',
      );
      if (!extractedTitle || extractedTitle.length) {
        try {
          // Selection des images par le titre et des mots-clés
          const prompt =
            this.getPromptsService.getPromptGenericSelectKeyWordsFromChapitresInArticle(
              extractedTitle,
              chapitreKeyWordList,
            );
          ...
          try {
            ...
            if (response && response.length > 0) {
              const keyWord = response.match(/\{"keyWord":"\{?[^}]+\}?"\}/g);
              if (keyWord && keyWord.length > 0) {
                const test2 = extractJSONBlock(keyWord[0]);
            ...
            console.error(
              "Erreur lors de l'analyse du JSON pour les mots-clés du chapitre :",
              parseError,
            );
            continue;
          }
          ...
          if (!respImagesUrl || !respImagesUrl.regularUrls) {
            console.error(
              "Erreur: Aucune URL d'image trouvée pour les mots-clés:",
              chapitreKeyWord,
            );
            continue;
          }
          // Sélection de la meilleure image pour le chapitre
          const bestImagePrompt =
            this.getPromptsService.getPromptGenericSelectBestImageForChapitresInArticle(
              extractedParagraphe,
              respImagesUrl.regularUrls,
            );
          ...
          try {
            ...
            console.error(
              "Erreur lors de la sauvegarde de l'URL de l'image dans Supabase :",
              saveError,
            );
          }
        } catch (error) {
          console.error(
            "Erreur lors du processus de récupération des mots-clés et d'image :",
            error,
          );
        }
      } else {
